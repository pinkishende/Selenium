...
public class Case1_Main_TestNG {
  
  WebDriver driver;
  @Test(dataProvider = "dp")
  public void f(String fname, String lname, String email,String password, String cpassword ) throws IOException, Exception
  {	
	  ...
	  Case1_POM_File obj = new Case1_POM_File(driver);
	  String title= driver.getTitle();
		Assert.assertEquals(title, "magento");
		
		ExtentReports extent = new ExtentReports();
		ExtentSparkReporter spark = new ExtentSparkReporter("Case1.html");
		extent.attachReporter(spark);
		ExtentTest test= extent.createTest("Verfiy the title of magento");
		
		if(title.equals("magento")) 
		{//     change o here
			test.pass("magento  title is displayed");
		}
		else {
			Thread.sleep(3000);
			...
			test.fail("magento  title is not displayed"
					,MediaEntityBuilder
					.createScreenCaptureFromPath(path).build());
			test.addScreenCaptureFromPath(path);
		}
		
		obj.createaccountclick();
		obj.firstname(fname);
		obj.lastname(lname);
		obj.email(email);
  ...
  public void beforeMethod() throws Exception {
	 	...
	 	InputStream input1 = new FileInputStream("C:\\Users\\pinki.shende\\eclipse-workspace\\Jan2024_Selenium\\src\\test\\java\\Jan24_TestNG\\Case1.properties");
